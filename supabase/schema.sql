...
-- Run in Supabase SQL editor or via pnpm setup-admin

-- ============ OFFERINGS (base table — idempotent) ============
...
drop policy if exists "public read active jobs" on public.job_openings;
create policy "public read active jobs" on public.job_openings for select using (status = 'active');

drop policy if exists "public read active kb" on public.chatbot_kb;
