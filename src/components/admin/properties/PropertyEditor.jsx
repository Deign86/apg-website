...
export default function PropertyEditor({ row, transactionTypes, onClose, onSaved, toast }) {
  ...
  return <div className="admin-dialog-overlay" onClick={onClose}>
    <div className="admin-dialog-box property-editor-dialog" onClick={(event) => event.stopPropagation()}>
      ...
      {row?.drive_folder_id && <aside className="property-provenance"><strong>Imported from Drive</strong><a href={`https://drive.google.com/drive/folders/${encodeURIComponent(row.drive_folder_id)}`} target="_blank" rel="noreferrer">Open source folder</a><span>Imported {row.imported_at ? new Date(row.imported_at).toLocaleString() : 'date unavailable'}</span><span>Last import batch: {row.import_batch_id || 'not recorded'}</span><small>Drive changes do not update this listing automatically. Make corrections in APG Admin.</small></aside>}
